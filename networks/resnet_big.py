...
class ResNet(nn.Layer):
    def __init__(self, block, num_blocks, in_channel=3, zero_init_residual=False):
        ...
        self.conv1 = nn.Conv2D(in_channel, 64, kernel_size=3, stride=1, padding=1, bias_attr=False)
        ...
        for m in self.sublayers():
            if isinstance(m, nn.Conv2D):
                m.weight = paddle.create_parameter(shape=m.weight.shape, dtype='float32', default_initializer=paddle.nn.initializer.KaimingNormal())
        ...
        if zero_init_residual:
            for m in self.sublayers():
                if isinstance(m, Bottleneck):
                    m.bn3.weight = paddle.create_parameter(shape=m.bn3.weight.shape, dtype='float32', default_initializer=paddle.nn.initializer.Constant(0.0))
                elif isinstance(m, BasicBlock):
                    m.bn2.weight = paddle.create_parameter(shape=m.bn2.weight.shape, dtype='float32', default_initializer=paddle.nn.initializer.Constant(0.0))
...
class LinearBatchNorm(nn.Layer):
